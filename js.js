...
class Cloud {
  constructor(imagesrc, x, y, height,width) {
...
function update(modifier){
 ...
 {    
  ...
  for(var j = Cloud_Array.length - 1; j >= 0; j--)
  {    
   if(Cloud_Array[j].x < 800 && Cloud_Array[j].y < 800 && Cloud_Array[j].x > 0 - 100 && Cloud_Array[j].y > 0 - 100)
   {
     c.drawImage(clouds,Cloud_Array[j].x, Cloud_Array[j].y, Cloud_Array[j].height, Cloud_Array[j].width);
   }
 }
...
function Load_Clouds(){
  ...
  for(var x = 0; x <= 100; x++)
  {
    Cloud_Array.push(new Cloud("img/Mist.png",getRndInteger(-400,800),getRndInteger(-400,800),500,100, 400));
  }
}
